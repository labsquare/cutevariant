...
class Node(object):
    ...
    def set_node_id(self, node_id=None):
        ...
        req = Node.conn.execute(
            f"SELECT nodes.id as 'node_id', terms.id as 'term_id', nodes.left, nodes.right, nodes.depth, terms.name, terms.hpo FROM nodes, terms WHERE nodes.term_id = terms.id AND nodes.id = {node_id}"
        )
        result = req.fetchone()
...
conn = sqlite3.connect("/home/sacha/hpo.db")
