...
def test_vql_command(conn):
    ...
    cmd = command.cmd_from_vql(conn, next(vql.execute_vql("SELECT chr, pos FROM variants")))
    
    assert type(cmd) == command.SelectCommand
    assert cmd.columns == ["chr","pos"]
    assert cmd.source == "variants"

    cmd = command.cmd_from_vql(conn,  next(vql.execute_vql("CREATE denovo FROM variants")))
