...
def fields_to_sql_refactored(fields):
    """Return field as SQL syntax
    ...
    `variants`.`pos`,
    `variants`.`ref`,
    `annotations`.`gene` AS ann.gene,
    `annotations`.`impact` AS ann.impact,
    `sample_boby`.`gt` AS sample.boby.gt,
    `sample_boby`.`dp` AS sample.boby.dp,
    `sample_charles`.`gt` AS sample.charles.gt,
    "]
    ...
    for field in fields:
        ...
        if field.startswith("ann."):
            ...
            sql_field = f"`sample_{name}`.`{value}`"
            sql_field_alias = f"`sample.{name}.{value}`"
            sql_fields.append((sql_field, sql_field_alias))
...
def build_count_unique(
    ...
    """Builds a SQL query to count unique values of a field
    For now, you can only do groupby on one field at a time (fields can be a list, though it will only take into account one)
    """
    ...
    # TODO: Try with sql_fields = fields_to_sql(fields, use_as=True) and then SELECT several counts with several GROUPBYs

    sql_query = f"SELECT {sql_fields[0]} AS {field_aliases[0]}, COUNT({sql_fields[0]}) AS count "
    ...
    sql_query += f" GROUP BY {sql_fields[0]}"

    # TODO : sqlite escape field with quote
    orientation = "DESC" if order_desc else "ASC"
    sql_query += (
        f" ORDER BY count {orientation}"
        if order_by_count
        else f" ORDER BY {sql_fields[0]} {orientation}"
    )
