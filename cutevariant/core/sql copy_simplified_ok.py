...
def insert_variants(
    ...
    for variant_count, variant in enumerate(variants):
        ...
        query_nb_samples = f"""UPDATE variants SET `nb_samples`={nb_samples} WHERE id={sample["variant_id"]}"""
        #print(query_nb_samples)
...
def create_triggers(conn):
        ...
        conn.execute(
            ...
            BEGIN
                ...
                SET count_none = count_none + IIF(old.gt = -1, -1, 0),
                    count_ref = count_ref + IIF(old.gt = 0, -1, 0),
                    count_het = count_het + IIF(old.gt = 1, -1, 0),
                    count_hom = count_hom + IIF(old.gt = 1, -1, 0),
                    count_none = count_none + IIF(new.gt = -1, 1, 0),
                    count_ref = count_ref + IIF(new.gt = 0, 1, 0),
                    count_het = count_het + IIF(new.gt = 1, 1, 0),
                    count_hom = count_hom + IIF(new.gt = 1, 1, 0)
                WHERE id = new.variant_id;
            END;
            """
        )
    ...
    # conn.execute(
    #     """
    #     CREATE TRIGGER count_calculation_after_update_variants AFTER UPDATE ON variants
    #     BEGIN
    #         UPDATE variants
    #         SET count_ref = (SELECT count(variant_id) FROM sample_has_variant WHERE variant_id=new.id AND gt=0),
    #             count_het = (SELECT count(variant_id) FROM sample_has_variant WHERE variant_id=new.id AND gt=1),
    #             count_hom = (SELECT count(variant_id) FROM sample_has_variant WHERE variant_id=new.id AND gt=2)
    #         WHERE variants.id = new.id ;
    #     END;
    #     """
    # )
    ...
    conn.execute(
        """
        CREATE TRIGGER freq_calculation_after_update_variants AFTER UPDATE ON variants
        WHEN new.nb_samples <> old.nb_samples
        BEGIN
            UPDATE variants
