...
def create_triggers(conn):
    ...
    conn.execute(
        ...
        BEGIN
        UPDATE variants SET count_var = new.count_het + new.count_hom WHERE variants.id = new.id ;

        END;"""
    )

    # For insert in sample_has_variant
    if True:
        conn.execute(
            """
    ...
    # For Update in sample_has_variant
    if True:
        ...
        if False:
            ...
            conn.execute(
                """
                CREATE TRIGGER count_het_from_none_after_update AFTER UPDATE ON sample_has_variant
                WHEN new.gt = 1 
                    AND old.gt = -1
                BEGIN
                    UPDATE variants
            ...
            conn.execute(
                """
                CREATE TRIGGER count_het_from_ref_after_update AFTER UPDATE ON sample_has_variant
                WHEN new.gt = 1 
                    AND old.gt = 0
                BEGIN
                    UPDATE variants
            ...
            conn.execute(
                """
                CREATE TRIGGER count_het_from_hom_after_update AFTER UPDATE ON sample_has_variant
                WHEN new.gt = 1 
                    AND old.gt = 2
                BEGIN
                    UPDATE variants
            ...
            conn.execute(
                """
                CREATE TRIGGER count_hom_from_het_after_update AFTER UPDATE ON sample_has_variant
                WHEN new.gt = 2 
                    AND old.gt = 1
                BEGIN
                    UPDATE variants
            ...
            conn.execute(
                """
                CREATE TRIGGER raw_from_het_after_update AFTER UPDATE ON sample_has_variant
                WHEN new.gt = 0 
                    AND old.gt = 1
                BEGIN
                    UPDATE variants
            ...
            conn.execute(
                """
                CREATE TRIGGER raw_from_hom_after_update AFTER UPDATE ON sample_has_variant
                WHEN new.gt = 0 
                    AND old.gt = 2
                BEGIN
                    UPDATE variants
            ...
            conn.execute(
                """
                CREATE TRIGGER null_from_het_after_update AFTER UPDATE ON sample_has_variant
                WHEN new.gt = -1 
                    AND old.gt = 1
                BEGIN
                    UPDATE variants
            ...
            conn.execute(
                """
                CREATE TRIGGER null_from_hom_after_update AFTER UPDATE ON sample_has_variant
                WHEN new.gt = -1 
                    AND old.gt = 2
                BEGIN
                    UPDATE variants
    ...
    if True:
        # conn.execute(
        #     """
        #     CREATE TRIGGER sample_nb_update_on_variants AFTER UPDATE ON variants 
        #     WHEN new.count_hom <> old.count_hom
        #         OR new.count_het <> old.count_het
        #     BEGIN

        #     UPDATE variants SET
        #         nb_samples = (SELECT count(id) FROM samples);
        ...
        conn.execute(
            ...
            UPDATE variants SET
                freq_var = ( cast((new.count_hom*2 + new.count_het) as real) / (nb_samples*2) )
            WHERE variants.id = new.id ;
