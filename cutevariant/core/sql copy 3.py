...
def create_triggers(conn):
    ...
    # count_none phenotype
    conn.execute(
        """
        CREATE TRIGGER count_none_after_insert_phenotype AFTER INSERT ON sample_has_variant
        WHEN new.gt = -1 
            AND (SELECT samples.phenotype FROM samples WHERE samples.id=new.sample_id) >= 1
        BEGIN
            UPDATE variants
            SET control_count_none = control_count_none + IIF((SELECT samples.phenotype FROM samples WHERE samples.id=new.sample_id) = 1, 1, 0),
    ...
    conn.execute(
        """
        CREATE TRIGGER count_ref_after_insert_phenotype AFTER INSERT ON sample_has_variant
        WHEN new.gt = 0 
            AND (SELECT samples.phenotype FROM samples WHERE samples.id=new.sample_id) >= 1
        BEGIN
            UPDATE variants
            SET control_count_ref = control_count_ref + IIF((SELECT samples.phenotype FROM samples WHERE samples.id=new.sample_id) = 1, 1, 0),
    ...
    conn.execute(
        """
        CREATE TRIGGER count_het_after_insert_phenotype AFTER INSERT ON sample_has_variant
        WHEN new.gt = 1 
            AND (SELECT samples.phenotype FROM samples WHERE samples.id=new.sample_id) >= 1
        BEGIN
            UPDATE variants
            SET control_count_het = control_count_het + IIF((SELECT samples.phenotype FROM samples WHERE samples.id=new.sample_id) = 1, 1, 0),
    ...
    conn.execute(
        """
        CREATE TRIGGER count_hom_after_insert_phenotype AFTER INSERT ON sample_has_variant
        WHEN new.gt = 2 
            AND (SELECT samples.phenotype FROM samples WHERE samples.id=new.sample_id) >= 1
        BEGIN
            UPDATE variants
            SET control_count_hom = control_count_hom + IIF((SELECT samples.phenotype FROM samples WHERE samples.id=new.sample_id) = 1, 1, 0),
    ...
    # count_none control phenotype 1
    conn.execute(
        """
        CREATE TRIGGER count_none_after_update_phenotype_control AFTER UPDATE ON sample_has_variant
        WHEN new.gt = -1 
            AND old.gt <> new.gt
            AND (SELECT samples.phenotype FROM samples WHERE samples.id=new.sample_id) = 1
        BEGIN
            UPDATE variants
            SET control_count_none = control_count_none + 1,
                control_count_ref = control_count_ref + IIF(old.gt = 0, -1, 0),
                control_count_het = control_count_het + IIF(old.gt = 1, -1, 0),
                control_count_hom = control_count_hom + IIF(old.gt = 2, -1, 0)
            WHERE id = new.variant_id;
        END;
        """
    )

    # count_none case phenotype 2
    conn.execute(
        """
        CREATE TRIGGER count_none_after_update_phenotype_case AFTER UPDATE ON sample_has_variant
        WHEN new.gt = -1 
            AND old.gt <> new.gt
            AND (SELECT samples.phenotype FROM samples WHERE samples.id=new.sample_id) = 2
        BEGIN
            UPDATE variants
            SET case_count_none = case_count_none + 1,
                case_count_ref = case_count_ref + IIF(old.gt = 0, -1, 0),
                case_count_het = case_count_het + IIF(old.gt = 1, -1, 0),
                case_count_hom = case_count_hom + IIF(old.gt = 2, -1, 0)
            WHERE id = new.variant_id;
        END;
        """
    )
    ...
    # count_ref control phenotype 1
    conn.execute(
        """
        CREATE TRIGGER count_ref_after_update_phenotype_control AFTER UPDATE ON sample_has_variant
        WHEN new.gt = 0 
            AND old.gt <> new.gt
            AND (SELECT samples.phenotype FROM samples WHERE samples.id=new.sample_id) = 1
        BEGIN
            UPDATE variants
            SET control_count_none = control_count_none + IIF(old.gt = -1, -1, 0),
    ...
    # count_ref case phenotype 2
    conn.execute(
        """
        CREATE TRIGGER count_ref_after_update_phenotype_case AFTER UPDATE ON sample_has_variant
        WHEN new.gt = 0 
            AND old.gt <> new.gt
            AND (SELECT samples.phenotype FROM samples WHERE samples.id=new.sample_id) = 2
        BEGIN
            UPDATE variants
            SET case_count_none = case_count_none + IIF(old.gt = -1, -1, 0),
    ...
    conn.execute(
        """
        CREATE TRIGGER count_het_after_update_control_phenotype AFTER UPDATE ON sample_has_variant
        WHEN new.gt = 1 
            AND old.gt <> new.gt
            AND (SELECT samples.phenotype FROM samples WHERE samples.id=new.sample_id) = 1
        BEGIN
    ...
    conn.execute(
        """
        CREATE TRIGGER count_het_after_update_case_phenotype AFTER UPDATE ON sample_has_variant
        WHEN new.gt = 1 
            AND old.gt <> new.gt
            AND (SELECT samples.phenotype FROM samples WHERE samples.id=new.sample_id) = 2
        BEGIN
    ...
    # count_hom control phenotype
    conn.execute(
        """
        CREATE TRIGGER count_hom_after_update_control_phenotype AFTER UPDATE ON sample_has_variant
        WHEN new.gt = 2 
            AND old.gt <> new.gt
            AND (SELECT samples.phenotype FROM samples WHERE samples.id=new.sample_id) = 1
        BEGIN
    ...
    # count_hom case phenotype
    conn.execute(
        """
        CREATE TRIGGER count_hom_after_update_case_phenotype AFTER UPDATE ON sample_has_variant
        WHEN new.gt = 2 
            AND old.gt <> new.gt
            AND (SELECT samples.phenotype FROM samples WHERE samples.id=new.sample_id) = 2
        BEGIN
