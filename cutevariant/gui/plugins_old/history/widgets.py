...
class HistoryModel(QAbstractTableModel):
    ...
    history_started = Signal()
    groubpby_finished = Signal()
    history_error = Signal()
    ...
    def __init__(self, parent: QObject = None, conn: sqlite3.Connection = None) -> None:
        ...
        self._raw_data = []
        self._conn = conn
        self.load_history_thread = SqlThread(self._conn)
        self.load_history_thread.started.connect(self.history_started)
        self.load_history_thread.finished.connect(self.groubpby_finished)
        self.load_history_thread.result_ready.connect(self._on_data_available)
        self.load_history_thread.error.connect(self._on_error)

        self._field_name = "chr"

        self._fields = ["user", "table", "field"]
    ...
    def data(self, index: QModelIndex, role: int):
        ...
        print(self._raw_data[index.column()][self._field_name])
    ...
    def set_conn(self, conn: sqlite3.Connection):
        self._conn = conn
        self.load_history_thread.conn = self._conn
    ...
    def load(
        ...
        #history_func = lambda conn: sql.get_variant_as_group(
        history_func = lambda conn: sql.get_history(
            conn,
            self._field_name,
            self._fields,
            self._source,
        ...
        self.load_history_thread.start_function(lambda conn: list(history_func(conn)))
        self.is_loading = True

    def _on_data_available(self):
        self._set_raw_data(self.load_history_thread.results)
        self.is_loading = False
    ...
    def _on_error(self):
        QMessageBox.critical(
            ...
            self.tr("Error!"),
            self.tr(
                f"Group by thread returned error {self.load_history_thread.last_error}"
            ),
        )
        self.clear()
...
class HistoryTable(QWidget):
    def __init__(self, conn=None, parent=None):
        ...
        self.proxy.setSourceModel(self.history_model)
        self.tableview.setSelectionMode(QAbstractItemView.SingleSelection)
        self.tableview.setSelectionBehavior(QAbstractItemView.SelectRows)

        self.history_model.history_started.connect(self.start_loading)
        self.history_model.groubpby_finished.connect(self.stop_loading)
    ...
    def conn(self, conn):
        ...
        self._conn = conn
        if conn:
            self.history_model.set_conn(conn)
...
class HistoryWidget(PluginWidget):
    ...
    def __init__(self, parent=None, conn=None):
        ...
        self.field_select_combo.currentTextChanged.connect(self._load_history)
        ...
        # Create view
        self.view = HistoryTable(conn, self)
        ...
        self.view.history_model.history_started.connect(
            lambda: self.field_select_combo.setEnabled(False)
        )
        self.view.history_model.groubpby_finished.connect(
    ...
    def on_double_click(self):
        ...
        if self.mainwindow:
            self.add_condition_to_filters(
                {self.view.history_model.get_field_name(): selected_value}
            )
    ...
    def on_apply(self):
        ...
        if selected_values:
            self.add_condition_to_filters(
                {self.view.history_model.get_field_name(): {"$in": selected_values}}
            )
