...
# Custom imports
from cutevariant.core.importer import async_import_file, async_update_file
...
class ProjectPage(QWizardPage):
    ...
    def __init__(self):
        ...
        self.setTitle(self.tr("Project update"))
        self.setSubTitle(
            self.tr("This wizard will guide you to update a cutevariant project with an additional VCF.")
        )

        # Reload last directory used
        app_settings = QSettings()
        self.last_directory = app_settings.value("last_directory", QDir.homePath())
        # self.last_directory = os.path.dirname(os.path.abspath(
                                # self.conn.execute("PRAGMA database_list").fetchone()["file"]))

        self.project_name_edit = QLineEdit()
        self.project_path_edit = QLineEdit()
        self.project_name_edit.setText(
            os.path.basename(app_settings.value("recent_projects", list())[0])
        )
        self.project_path_edit.setText(app_settings.value("recent_projects", list())[0])
        ...
        v_layout.addRow(self.tr("Full Path"), browse_layout)
    ...
    def _browse(self):
        ...
        path = QFileDialog.getOpenFileName(
            self, self.tr("Select an existing project"), self.last_directory, self.tr("*.db")
        )
        print("path:", path)
        if path:
            path = path[0]
            self.project_path_edit.setText(path)
            self.last_directory = os.path.dirname(path)  # If you change your mind after this point, it's good to have the last directory you selected
            self.project_name_edit.setText(
    ...
    def isComplete(self):
        ...
        return (
            True
            if (
                QFile(self.project_path_edit.text()).exists()
...
class ImportThread(QThread):
    ...
    def run(self):
        ...
        try:
            # Import the file
            for value, message in async_update_file(
                self.conn,
                self.filename,
...
class ImportPage(QWizardPage):
    ...
    def initializePage(self):
        ...
        #     + ".db"
        # )
        self.db_filename = self.field("project_path")
...
class UpdateWizard(QWizard):
    """Main window of the update wizard
    (Adapted from the project wizard)
