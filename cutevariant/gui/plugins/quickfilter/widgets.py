...
class QuickFilterModel(QAbstractTableModel):
...
class QuickFilterTable(QWidget):
    def __init__(self, conn=None, parent=None):
        super().__init__(parent=parent)
        self.conn = conn
        self.groupby_model = QuickFilterModel(self)
...
class QuickfilterWidget(PluginWidget):
    ...
    def __init__(self, parent=None, conn=None):
        ...
        sources_list=self.sources_load()
        self.sources.addItems(sources_list)
        self.sources.currentIndexChanged.connect(self.sources_selectionchange)
    ...
    def sources_load(self):
        #sources=sql.get_selections(self.conn)
        #print(sources)
        # for data in self.conn.execute("SELECT * FROM selections"):
        #     print(data)
        return ["sample1", "sample2"]
    
    def sources_selectionchange(self,i):
