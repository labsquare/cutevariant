...
def get_variants(self,conn: sqlite3.Connection):
    """Get variants"""
    #print("TTEESSTT")
    #print(self.mainwindow.get_state_data("filters"))
    #print(self.mainwindow.get_state_data("source"))
    fields={"id", "chr", "pos", "ref", "alt", "classification", "is_snp", "is_indel"}
    filters=self.mainwindow.get_state_data("filters")
    source=self.mainwindow.get_state_data("source")
    #variants=sql.get_variants(self.conn, fields, source, filters, limit=None)
    #print(variants)

    results = {}
    for record in sql.get_variants(self.conn, fields, source, filters, limit=None):
        #print(record)
    ...
    #     f"""SELECT `id`, `chr`, `pos`, `ref`, `alt`, `classification`
    #         FROM `variants`
    #         """
    # ):
    #     results[record["id"]] = dict(record)

    # print("nb variants selected")
    # print(len(results))
    return results
    ...
    #def create_figure(self, test=1):
        ...
        if variants:
            for variant in variants:
                contig_list[variants[variant]["chr"]]=1 #=contig_list{variants[variant]["chr"]}+1
                classification_list[variants[variant]["classification"]]=1 #=contig_list{variants[variant]["chr"]}+1
                # if variants[variant]["is_indel"]:
                #     print("IS INDEL")
        ...
        for chr in contigs:
            ...
            #if len(contig_list) > 1 and contig_list[chr]: 
            if chr in contig_list: 
                ...
                arc    = Garc(arc_id=name, size=length, interspace=3, raxis_range=(850,900), labelposition=60, facecolor=arcdata_dict[name]["colors"], label_visible=True)
                circle.add_garc(arc) 
        ...
        if variants:
            ...
            for variant in variants:
                ...
                if variants[variant]["is_snp"] and 1:
                    #print("IS SNP")
                    ...
                    arcdata_dict[name]["positions"].append(mid) 
                    arcdata_dict[name]["values"].append(value)
                    #arcdata_dict[name]["colors"] = "#BBBBBB" # #ed6d79
                    #arcdata_dict[name]["colors"] = "#f5a26f" # #ed6d79
                    arcdata_dict[name]["colors"].append(style.CLASSIFICATION[classification].get("color"))
                    #print(arcdata_dict[name]["colors"])
            ...
            for key in arcdata_dict:
                ...
                #                rlim=[vmin-0.05*abs(vmin), vmax+0.05*abs(vmax)], raxis_range=(860,940), facecolor=arcdata_dict[key]["colors"], spine=True) 
                rlim=[vmin-0.05*abs(vmin), vmax+0.05*abs(vmax)], raxis_range=(760,840), facecolor=arcdata_dict[key]["colors"], spine=True) 


        # for cl in style.CLASSIFICATION:
        #     print(cl)
        #     print(style.CLASSIFICATION[cl].get("color"))
